...
struct ThirdTutorialView: View {
    ...
    @StateObject private var manager = ThirdTutorialOhajikiManager()
    ...
    var body: some View {
        GeometryReader { geometry in
            ZStack {
                ...
                VStack {
                    Spacer()
                    Text("Ohajiki of different colors do not merge.\nCollide yellow Ohajiki together to merge them.")
        ...
        .overlay(
            Group {
                if shouldNavigate {
                    FourthTutorialView(showBackButton: showBackButton)
                        .transition(.opacity)
                }
            }
        )
...
/// 第三チュートリアル用のおはじきマネージャー（手玉1つ、赤玉1つ、黄玉2つ）
class ThirdTutorialOhajikiManager: OhajikiManager {
    ...
    override func updatePhysics() {
        ...
        // 手玉以外の玉が3つから2つに減ったら合体を検出
        if nonHandBallCountBefore == 3 && nonHandBallCountAfter == 2 && !hasDetectedMerge {
    ...
    override func initializePositions() {
        // 親クラスの実装は呼ばない（手玉1つ、赤玉1つ、黄玉2つの配置）
        ...
        // 黄色い円2を左側の下に配置
        if circles.count > 3 {
            circles[3].offset = CGSize(width: -boundaryRadius * 0.6, height: 0)
        }
    }
}
